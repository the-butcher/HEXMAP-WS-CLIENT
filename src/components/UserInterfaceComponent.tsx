...
export default (props: IUserInterfaceProps) => {
  ...
  const availableIndicatorWidth = window.innerWidth - 26;
    ...
    <ThemeProvider theme={theme}>
      <div style={{ width: '100%', position: 'relative', display: 'flex', flexDirection: 'column', pointerEvents: 'none' }}>
        <div style={{ width: 'calc(100%-18px)', zIndex: 100, display: 'flex', flexDirection: 'row', flex: 1, padding: '2px 9px 9px 13px' }}>
          {indicatorProps.map(props => <IndicatorComponent key={`indicator_${props.id}`} {...props} style={{ minWidth: `${props.fold === 'open-horizontal' || props.fold === 'open-vertical' ? openedWidth : closedWidth}px`, display: 'block' }} />)}
        </div>
      </div>
    </ThemeProvider>
  );
